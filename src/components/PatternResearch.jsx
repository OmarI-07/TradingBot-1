...
export default function PatternResearch(){
  ...
  return <div className="card" style={{marginBottom:20}}>
    ...
    <details style={{marginTop:14}}><summary>Load months using your existing Massive connection</summary>
      <p>Select consecutive months. Your existing providerâ€™s permissions, rate limits, and data charges apply.</p>
      ...
      <button className="btn-sm" disabled={running||loading||!months.length} onClick={loadMonths}>{loading?'Loading candlesâ€¦':'Load selected months'}</button>
    </details>
    ...
    <p style={{fontSize:12,color:'var(--text-muted)'}}>Default cost assumption: $0.74 per contract per side, 1 tick entry/exit, 2 ticks on stops; verify your fees before relying on results.
      Regular-session entries only; flatten by 15:55 New York time. Risk and drawdown limits are research settings, not a funded firmâ€™s rules.</p>
